...
Page({
  ...
  wxChooseAddress: function() {
    ...
    wx.chooseAddress({
      success: function(s) {
        ...
        util.request(api.AddressDistrict, {
          provinceName: s.provinceName,
          cityName: s.cityName,
          districtName: s.countyName
        }).then(function(res) {
          if (res.errno === 0) {
              console.log(res);
              a.provinceId = res.data.province.id,
              a.cityId = res.data.city.id,
              a.areaId = res.data.district.id,
              a.name = s.userName,
              a.mobile = s.telNumber,
              a.provinceName = s.provinceName,
              a.cityName = s.cityName,
              a.areaName = s.countyName,
              a.address = s.provinceName + s.cityName + s.countyName + s.detailInfo,
              e.setData({
                address: a
              });
          } else {
  ...
  wxAddress: function() {
    ...
    wx.getSetting({
      success: function(a) {
        a.authSetting["scope.address"] ? e.wxChooseAddress() : wx.authorize({
          scope: "scope.address",
          success: function() {
            e.wxChooseAddress();
          }
